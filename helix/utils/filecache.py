...
def file_cache(ignore_params=[], verbose=False, cache_dir=None):
    def decorator(func):
        if DISABLE_CACHE:
            if verbose:
        ...
        def wrapper(*args, **kwargs):
            ...
            cache_file = os.path.join(
                default_cache_dir, f"{func.__module__}_{func.__name__}_{arg_hash}.pickle"
            )
            ...
            try:
                ...
                if os.path.exists(cache_file):
                    ...
                    if verbose:
                        logger.info(
                            f"No cache hit for function: {func.__name__}")
            except Exception:
